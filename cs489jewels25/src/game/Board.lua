...
Board.currentChain=0 --init chain count to 0 for later printing
function Board:init(x,y, stats)
    ...
    self.coins={}
    self.coinSpriteSheet = love.graphics.newImage("graphics/sprites/coin_spritesheet.png")
    local gridCoin = anim8.newGrid(16, 16,self.coinSpriteSheet:getWidth(),self.coinSpriteSheet:getHeight())
    self.coinAnimation = anim8.newAnimation(gridCoin("1-4", 1), 0.1)  -- Adjust speed with 0.1
    ...
    self:placeRandomCoin() --function to put coin on random position
end
function Board:placeRandomCoin()
    local row = math.random(1, Board.MAXROWS)--choose random row
    local col = math.random(1, Board.MAXCOLS)--choose random column
    self.coins[row .. "-" .. col] = {row = row, col = col}--place coin in random position
end
function Board:removeCoinIfMatched(matches)
    for _, match in pairs(matches) do --if a match
        for j = 0, match.size - 1 do
            local checkRow = match.row
            local checkCol = match.col + j
            self:checkAndRemoveCoin(checkRow, checkCol) -- if coin is adj to match, remove and inc score
        end
    end
end

function Board:checkAndRemoveCoin(row, col)
    local directions = {{0,1}, {0,-1}, {1,0}, {-1,0}} -- Right, Left, Down, Up

    for _, dir in pairs(directions) do
        local adjRow = row + dir[1]
        local adjCol = col + dir[2]
        if self.coins[adjRow .. "-" .. adjCol] then
            self.coins[adjRow .. "-" .. adjCol] = nil -- Remove coin
            Sounds["coin"]:play() -- Play a sound if a coin is removed
            score=score+1000
        end
    end
end
...
function Board:draw()
    ...
    for _, coin in pairs(self.coins) do
        self.coinAnimation:draw(
            ...
            self.x + (coin.col - 1) * Board.TILESIZE,
            self.y + (coin.row - 1) * Board.TILESIZE,
            0,  -- Rotation
            Board.TILESIZE / 32,  -- Scale X (assuming sprite is 32x32)
            Board.TILESIZE / 32   -- Scale Y
        )
    end
    self:drawChain()
end
...
function Board:matches()
    ...
    if #horMatches > 0 or #verMatches > 0 then -- if there are matches
        self.currentChain=self.currentChain + 1 --increment chain counter if consecutive matches are found
        ...
        self:removeCoinIfMatched(horMatches)--check if coin is next to match, and if so delete it
        self:removeCoinIfMatched(verMatches)
...
function Board:drawChain()
    if self.currentChain > 1 then
        love.graphics.setColor(1, 1, 1, 1)  -- White color
        love.graphics.print("Chain: " .. self.currentChain, self.x, self.y - 30)
    end
end
function Board:createExplosion(row,col)
    ...
    -- Default to white if no gem or type not found
    local color = {1, 1, 1}
    
    -- Check if the tile exists and has a type
    if self.tiles[row] and self.tiles[row][col] then
        local gemType = self.tiles[row][col].type
        color = colors[gemType] or color
    end
    
    exp:trigger(
        self.x+(col-1)*Board.TILESIZE+Board.TILESIZE/2,
...
function Board:drawChain()
    if self.currentChain > 0 then
        love.graphics.setColor(5, 3, 1) -- White color
        love.graphics.setFont(love.graphics.newFont(20)) -- Set font size
        love.graphics.printf("Chain! x"..self.currentChain, self.x, gameHeight - 50, Board.MAXCOLS * Board.TILESIZE, "center")
    end
end
