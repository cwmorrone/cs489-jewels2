...
function love.keypressed(key)
    ...
    elseif key == "r" and gameState == "over" then
        -- Restart the game if in the "over" state
        gameState = "start"
        -- Reset necessary variables here, such as score, level, high score, etc.
        highScore = 0  -- Example: reset high score (if applicable)
        love.load()
    end
end
...
function drawGameOverState()
    love.graphics.printf("GameOver",titleFont,0,50,
        gameWidth,"center")
    love.graphics.printf("High Score: " .. tostring(highScore), love.graphics.newFont(24), gameWidth/2 - 100, gameHeight/2 + 60, 200, "center")
